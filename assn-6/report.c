...
int Report_DoReport(Bank *bank, int workerNum){
  sem_wait(&(bank->main_report));
  bank->workers_amount--;
  if(bank->workers_amount > 0){
    sem_post(&(bank->main_report));
    sem_wait(&(bank->end_day));
    return 0;
  }
  Report *rpt = bank->report;
  ...
  if (rpt->numReports >= MAX_NUM_REPORTS) {
      ...
      /* end of the day */
      int workers = numWorkers - 1;
      while (workers--) sem_post(&(bank->end_day));
      bank->workers_amount = numWorkers;
      sem_post(&(bank->main_report));
      return -1;
  }
