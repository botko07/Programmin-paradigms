          ...
          int numWorkers)
{
  ...
  sem_init(&(bank->end_day),0,0);
  sem_init(&(bank->transfer),0,1);
  sem_init(&(bank->main_report),0,1);
...
Bank_Balance(Bank *bank, AccountAmount *balance)
{
  ...
  for(size_t i=0; i < bank->numberBranches; i++) 
    sem_wait(&(bank->branches[i].branch_operations));
  ...
  for (unsigned int branch = 0; branch < bank->numberBranches; branch++) {
    ...
    if (err < 0) {
      for(size_t i=0; i < bank->numberBranches; i++) 
        sem_post(&(bank->branches[i].branch_operations));
      return err;
    }
