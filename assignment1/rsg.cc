...
// The recursive method that fills text
void find_words(map<string, Definition> &grammar, Production &prod, vector<string> &answer){
    for(string word : prod){
      if(!grammar.count(word)) answer.push_back(word);
      else{
        Production newProd = grammar[word].getRandomProduction();
        find_words(grammar,newProd,answer);
      }
    }
}

// this method generates the amount of texts that is given as test parameter
void generate_text(map<string, Definition> &grammar,int test){
  for(int i=1; i <= test; i++){
      cout << "Version #" << i << " --------------------------\n";
      Definition starting = grammar["<start>"];
      Production prod = starting.getRandomProduction();
      vector<string> answer;
      find_words(grammar,prod,answer);
