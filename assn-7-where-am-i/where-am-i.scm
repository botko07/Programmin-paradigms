...
;; originy written by Nick Parlante
...
(define (find-for-each p lst)
  (if (or (null? lst) (null? (car lst))) '()
    (append (intersect p (car lst)) (find-for-each p (cdr lst)))))


(define (intersection-points lst)
  (if (or (null? lst) (null? (cdr lst))) '()
    (append (find-for-each (car lst) (cdr lst)) (intersection-points (cdr lst)))))

(define (my-prod p1 p2)
  (if (and (= (x p1) (x p2)) (= (y p1) (y p2))) 1
    (dist p1 p2)))

(define (distance-product point lst)
  (apply * (map (lambda (x) (my-prod point x)) lst)))

(define (rate-points lst)
  (append (map (lambda (x) (cons (distance-product x lst) (list x))) lst) ))

(define (cmp p1 p2)
  (if (or (null? p2) (< (car p1) (car p2)) ) #t
    #f))

(define (my-partition pivot num-list)
  (if (null? num-list) '(() ())
      (let ((split-of-rest (my-partition pivot (cdr num-list))))
	(if (cmp (car num-list) pivot)
	    (list (cons (car num-list) (car split-of-rest)) (cadr split-of-rest))
	    (list (car split-of-rest) (cons (car num-list) (car (cdr split-of-rest))))))))


(define (my-quicksort num-list)
  (if (<= (length num-list) 1) num-list
      (let ((split (my-partition (car num-list) (cdr num-list))))
	(append (my-quicksort (car split)) 
		(list (car num-list)) 
		(my-quicksort (cadr split))))))

(define (sort-points lst)
  (my-quicksort lst))

(define (take-first-half lst len index)
  (if (= index len) (list (y (x lst)))
    (cons (y (x lst) ) (take-first-half (cdr lst) len (+ 1 index)))))


(define (clumped-points lst)
  (let ((seq (my-quicksort (rate-points lst))))
    (take-first-half seq (quotient (length seq) 2) 1)))

(define (average-point lst)
  (let ((average-x (/ (apply + (map (lambda (x) (car x)) lst)) (length lst))))
  (let ((average-y (/ (apply + (map (lambda (x) (cadr x)) lst)) (length lst))))
    (cons (distance-product (list average-x average-y) lst) (list (list average-x average-y))))))

(define (best-estimate lst)
  (average-point (clumped-points (intersection-points lst))))

(define (where-am-i dists lst)
  (my-quicksort (map best-estimate (all-guesses dists lst))))
